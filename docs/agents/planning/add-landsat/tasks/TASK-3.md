...
(1) Refactor `AbstractStacWrapper.__init__` to accept a `catalog_name` parameter (instead of hardcoding `PLANETARY_COMPUTER`), so subclasses do not have to create-then-discard a Planetary Computer client. (2) Implement the `AbstractLandsat` base class that targets the new `USGS_LANDSAT` catalog.
...
- **Code to refactor:**
    ...
    - `src/geospatial_tools/stac/core.py:432-433` — `StacSearch.__init__` initializes an S3 client only when `catalog_name == COPERNICUS`. Confirm new `USGS_LANDSAT` catalog does NOT trigger this branch (no change required if the conditional already guards correctly; verify and add a regression test).
...
2. Verify `StacSearch.__init__` (`core.py:432-433`) only initializes the S3 client for `COPERNICUS`. Add a unit test `StacSearch(USGS_LANDSAT).s3_client is None` to prevent future regressions.
3. Create `src/geospatial_tools/stac/usgs_landsat/landsat.py`.
4. Implement `AbstractLandsat(AbstractStacWrapper)`:
    - Override `__init__` to call `super().__init__(catalog_name=USGS_LANDSAT, ...)` and to default `collection` to `UsgsLandsatLandsatCollection.LEVEL_1_COLLECTION_2`.
    - Provide an abstract method or class attribute `_PLATFORM: ClassVar[UsgsLandsatLandsatPlatform]` that concrete classes (TASK-4) supply.
    - Implement `_build_collection_query(self) -> dict[str, Any]` that returns `{UsgsLandsatLandsatProperty.PLATFORM.value: {"eq": self._PLATFORM.value}}` plus any `custom_query_params`.
5. Create `tests/test_usgs_landsat_landsat.py` (shared with TASK-4) verifying:
    ...
    - When subclassed for the test, `self.client.catalog_name == USGS_LANDSAT`.
...
- `self.client.catalog_name` must equal `USGS_LANDSAT` for any `AbstractLandsat` subclass.
...
- `AbstractLandsat` targets the UsgsLandsat catalog.
- Direct instantiation of `AbstractLandsat` raises `TypeError`.
- `StacSearch(USGS_LANDSAT).s3_client is None`.
