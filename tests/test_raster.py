...
def test_reproject_raster(test_raster) -> None:
...
def test_clip_raster_with_polygon_single_polygon(test_raster, polygon_layer) -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
        output = clip_raster_with_polygon(test_raster, polygon_layer=polygon_layer, output_dir=tmp_dir)
        assert len(output) == 1


def test_clip_raster_with_polygon_three_polygons(test_raster, three_polygons_layer) -> None:
    with tempfile.TemporaryDirectory() as tmp_dir:
